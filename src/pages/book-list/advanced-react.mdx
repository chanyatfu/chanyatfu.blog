...
- If we pass `<A />` as a props to `<B />`, and `<B />` re-rendered. `<A />` would not nesessary re-render as its reference defined in the parent components. Only when `<A />` parent or `<A />` itself re-rendered, `<A />` would re-render.
